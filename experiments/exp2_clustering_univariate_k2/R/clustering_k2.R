...
kmeans_fr <- function(params, k, max_iter = 100L, n_init = 10L) {
  ...
  for (init in seq_len(n_init)) {
    ...
    labels  <- integer(n)          # 0-initialised; max.col returns 1:k
    ...
    for (iter in seq_len(max_iter)) {
      ...
      if (identical(new_labels, labels)) {
        labels <- new_labels          # redundant but makes post-loop state explicit
        break
      }
      labels <- new_labels
...
match_accuracy <- function(true_labels, pred_labels) {
  # For k=2: both label-to-cluster assignments are tried; best accuracy returned.
  # Relies on the identity max(acc, 1-acc) for two-class balanced problems.
...
run_replications <- function(mu_list, sigma_list, k = 2L, t = 100L, n = 30L,
  ...
  for (r in seq_len(n_rep)) {
    ...
    results[[r]] <- data.frame(
      replication = r,
      km_fr       = match_accuracy(true_labels, km_fr),
      km_eu       = match_accuracy(true_labels, km_eu),
      hc_fr       = match_accuracy(true_labels, hc_fr),
      hc_eu       = match_accuracy(true_labels, hc_eu)
    )
  }
